...
当内核执行 C 程序时（使用一个 exec 函数），在调用 main 前先调用一个特殊的启动例程。
启动例程从内核取得命令行参数和环境变量值，然后参数传递给 main 函数执行。

有 8 种方式使进程终止，正常终止的方式有 5 种：

1. 从 main 返回。
1. 调用 `exit` 先执行清理，再返回内核。
1. 调用 `_exit` 或 `_Exit` 立即进入内核。
1. 最后一个线程从其启动例程返回。
1. 从最后一个线程调用 `pthread_exit`。
...
int main(){
        printf("Hello, world\n");
        return 1; // 可以使用 exit(1)替代 return 1 。
}
```
...
atexit 的参数是一个无参无返回值的函数地址；atexit 的返回值为 0 则成功，非 0 则出错。
...
```bash
$ chapter07 gcc atexit.c ../error.c
$ chapter07 ./a.out
main is done
second exit hander.count = 1
second exit hander.count = 2
second exit hander.count = 3
```
...
可以发现未初始化段内容并不存放在磁盘文件种，需要存放在磁盘程序文件种的段只有正文段和初始化数据段。
...
1. 减小了可执行文件的大小。
1. 可以直接使用新版本的动态库替换旧版本。

动态库的缺点：

1. 程序每次运行时，增加了程序运行时间开销。
1. 不同版本的系统动态库可能不兼容，导致程序运行失败。
...
3 个函数返回值：若成功，返回非空指针；若出错，返回 NULL。返回的指针一定是适当对齐的，使其可用于任何数据对象。

函数 free 释放 ptr 指向的存储空间。

这些分配例程通常用 sbrk(2)系统调用实现扩充或缩小进程的堆。

虽然 sbrk 可以扩充或缩小进程的存储空间，但是大多数 malloc 和 free 的
实现都不减小进程的存储空间。释放的空间可供以后再分配，但将它们保
持在 malloc 池中而不返回给内核。
...
`getenv` 函数从环境中取一个指定环境变量的值。返回值：指向与 name 关联的 value 的指针；若未找到，返回 NULL。

`putenv` 函数以覆盖的模式设置幻境变量，返回值：若成功，返回 0；若出错，返回非 0。

`setenv` 和 `unsetenv` 两个函数的作用为将将 name 设置为 value 和删除 name 的定义。返回值：若成功，返回 0；若出错，返回 −1。

`clearenv` 函数用来删除环境表中的所有项，返回值：若成功，返回 0；若出错，返回非 0。
...
1. 修改一个现有的 name:
   ...
   1. 新 value 的长度大于现有 value 的长度，先调用 malloc 分配空间，然后复制到新空间，接着使环境表指向新地址。
2. 新增一个新的 name:
   1. 如果使第一次新增，则需要调用 malloc 为新的指针表分配空间。接着将原来环境表复制到新分配表，将新增的 name=value 的指针移至指针表的表尾，然后将一个空指针存放在其后，最后使 environ 指向新指针表。
   1. 如果不是第一次新增一个新的 name，只要调用 realloc，分配比原来空间多存放一个指针的空间，然后将指向新 name=value 的字符串的指针放在该表表尾，后面跟着一个空指针。
...
`setjmp` 参数 `env` 的类型是一个特殊类型 `jmp_buf`。这一数据类型是某种形式的数组，其中存放在调用 `longjmp` 时能用来恢复栈状态的所有信息。通常将 `env` 变量定义为全局变量。

`setjmp` 返回值：
- 若直接调用，返回 0；若从 `longjmp` 返回，则为非 0。

`longjmp` 函数第一个就是在调用 `setjmp` 时所用的 `env`；第二个参数是具非 0
值的 val，它将成为从 `setjmp` 处返回的值。使用第二个参数的原因是对于一
个 `setjmp` 可以有多个 `longjmp`。
...
1. 任何一个进程都可降低其硬限制值，但它必须大于或等于其软限制值。这种降低，对普通用户而言是不可逆的。
1. 只有超级用户进程可以提高硬限制值。
...
RLIMIT_NPROC        15411      15411
RLIMIT_RSS     (infinite) (infinite)
RLIMIT_SIGPENDING      15411      15411
